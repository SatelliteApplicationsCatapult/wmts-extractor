...
class Downloader:
    ...
    def get_tile_min_max(self, bbox, zoom):
        ...
        x1, y1 = self._tiler.lat_lon_to_tile(bbox[1], bbox[0], zoom)
        x2, y2 = self._tiler.lat_lon_to_tile(bbox[3], bbox[2], zoom)
    ...
    def process(self, uri, aoi, args, out_pathname):
        ...
        x1, y1, x2, y2 = self.get_tile_min_max(aoi.geometry.bounds, args.zoom)

        # get tasklist for threads
        threads = []
        tasklist = self.get_task_list(uri, (x1, y1, x2, y2), args.zoom, out_path)
    ...
    def get_task_list(self, uri, bbox, zoom, out_path):
        ...
        tasks = [[] for t in range(self._threads)]
        x1, y1, x2, y2 = bbox
        ...
        for y in range(min(y1, y2), max(y1, y2) + 1):
            for x in range(x1, x2 + 1):
                ...
                # append job to list
                tasks[index].append({'xyz': (x, y, zoom), 'uri': _uri, 'pathname': pathname})
                index += 1
